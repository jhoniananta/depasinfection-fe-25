...
import { participantsOKGD } from "@/types/dashboard-admin";
import Link from "next/link";
import { MdOutlineNavigateNext } from "react-icons/md";
import { useAdminOKGDDataQuery } from "../_hooks/@get/useAdmin";

export default withAuth(AdminOKGDDashboardPage, "ADMIN");
function AdminOKGDDashboardPage() {
  ...
  const {
    ...
    isLoading,
    // isError,
  } = useAdminOKGDDataQuery(currentPage, itemsPerPage, apiSearchKeyword);
  React.useEffect(() => {
    const pageParam = searchParams.get("page");
    const limitParam = searchParams.get("limit");
    if (!pageParam || !limitParam) {
      router.replace(`/admin/okgd?page=1&limit=10&search=`);
    }
  }, [searchParams, router]);
  ...
  const filteredData = React.useMemo(() => {
    ...
    return tableData.data.filter((item: participantsOKGD) => {
      const { team_name, school } = item;
  ...
  const handleSearchButtonClick = () => {
    ...
    router.replace(
      `/admin/okgd?page=${currentPage}&limit=${itemsPerPage}&search=${searchKeyword}`,
    );
  };

  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    router.replace(`/admin/okgd?page=${page}&limit=${itemsPerPage}&search=`);
  };
  ...
  const handleItemsPerPageChange = (key: string) => {
    ...
    router.replace(`/admin/okgd?page=1&limit=${limit}&search=`);
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <Sidebar title="OKGD Admin Dashboard">
          <section className="min-h-screen w-full overflow-hidden">
            ...
            <div className="container my-6 w-full">
              ...
              <div className="mb-5 grid grid-cols-1 gap-4 md:mb-10 md:grid-cols-2 xl:grid-cols-4">
                {/* Not Approved Yet */}
                <Card className="border-2 border-neutral-600 bg-neutral-200">
                  <CardContent className="flex flex-col items-center justify-center py-6 text-center">
                    <Typography className="text-[16px] text-neutral-800 md:text-[20px]">
                      Rejected
                    </Typography>
                    <Typography
                      variant="h4"
              ...
              <div className="mt-6 overflow-x-auto rounded-md border border-gray-200">
                <Table>
                  ...
                  <TableBody>
                    {filteredData.map(
                      (item: participantsOKGD, index: number) => (
                        <TableRow
                        ...
                        >
                          ...
                          <TableCell>
                            <Link
                              href={`/admin/okgd/${item.participant_id}`}
